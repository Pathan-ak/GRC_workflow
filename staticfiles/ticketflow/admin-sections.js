...
(function() {
    ...
    function initSectionAwareDrag() {
        ...
        // Update column indicator when section/column changes
        dataRows.forEach(row => {
            const sectionSelect = row.querySelector('select[name$="-section"]');
            const columnInput = row.querySelector('input[name$="-column"]');
            
            function updateColumnIndicator() {
                const sectionId = sectionSelect ? sectionSelect.value : '';
                const column = columnInput ? columnInput.value : '1';
                
                // Remove existing indicator
                const existing = row.querySelector('.column-indicator');
                ...
                if (sectionId && column) {
                    ...
                    indicator.textContent = `Col ${column}`;
                    
                    const labelCell = row.querySelector('td:first-of-type');
                    if (labelCell) {
                        labelCell.appendChild(indicator);
                    }
                }
            }
            
            if (sectionSelect) {
                sectionSelect.addEventListener('change', updateColumnIndicator);
            }
            if (columnInput) {
                columnInput.addEventListener('change', updateColumnIndicator);
            }
            
            // Initial update
            setTimeout(updateColumnIndicator, 200);
        });
